...
pub struct TypelessSparseSet {
    ...
    remove_ptr: fn(&mut Box<dyn Any>, EntityId),
}
...
impl TypelessSparseSet {
    pub fn new<T: 'static>(sparse_set: SparseSet<T>) -> Self {
        Self {
            ...
            remove_ptr: |sparse_set, entity_id| unsafe { sparse_set.downcast_mut::<SparseSet<T>>().unwrap_unchecked() }.remove(entity_id),
        }
    }

    #[inline]
    pub unsafe fn downcast_unchecked<T: 'static>(&self) -> &SyncUnsafeCell<SparseSet<T>> {
        // self.sparse_set.downcast_ref::<SyncUnsafeCell<SparseSet<T>>>()
        unsafe { self.sparse_set.downcast_ref_unchecked::<SyncUnsafeCell<SparseSet<T>>>() }
    }

    #[inline]
    pub fn remove(&mut self, entity_id: EntityId) {
        (self.remove_ptr)(&mut self.sparse_set, entity_id);
    }
}
...
impl<T> SparseSet<T> {
    ...
    pub fn contains(&self, entity_id: EntityId) -> bool {
        let sparse_index = self.sparse_array.get(entity_id);
    ...
    #[inline]
    pub fn get_ptr_unchecked(&self, entity_id: EntityId) -> *const T {
        let sparse_index = self.sparse_array.get(entity_id);
    ...
    #[inline]
    pub fn get_mut_ptr_unchecked(&mut self, entity_id: EntityId) -> *mut T {
        let sparse_index = self.sparse_array.get(entity_id);
        let index = sparse_index.0 as usize;
    ...
    #[inline]
    pub fn get_ptr(&self, entity_id: EntityId) -> Option<*const T> {
        let sparse_index = self.sparse_array.get(entity_id);
        let index = sparse_index.get()?;
        Some(&self.dense[index as usize])
    }
    ...
    #[inline]
    pub fn get_mut_ptr(&mut self, entity_id: EntityId) -> Option<*mut T> {
        let sparse_index = self.sparse_array.get(entity_id);
        let index = sparse_index.get()?;
        Some(&mut self.dense[index as usize])
    }
    ...
    #[inline]
    pub fn insert(&mut self, entity_id: EntityId, value: T) {
        let sparse_index = self.sparse_array.get(entity_id);
        if let Some(index) = sparse_index.get() {
            self.dense[index as usize] = value;
        } else {
            self.sparse_array.set(entity_id, SparseIndex::new(self.dense.len() as u32));
            self.dense.push(value);
            self.mapping.push(entity_id);
        }
    }
    ...
    pub fn remove(&mut self, entity_id: EntityId) {
        ...
        self.sparse_array.set(entity_id, SparseIndex::NONE);
...
impl SparseArray {
    #[inline]
    pub fn get(&self, entity_id: EntityId) -> SparseIndex {
        let id = entity_id.id() as usize;
        self.0.get(id).copied().unwrap_or(SparseIndex::NONE)
    }

    #[inline]
    pub fn set(&mut self, entity_id: EntityId, index: SparseIndex) {
        let id = entity_id.id() as usize;
        if id >= self.0.len() {
...
impl SparseIndex {
    ...
    #[inline]
    pub const fn set(&mut self, index: u32) {
        self.0 = index;
    }
}
