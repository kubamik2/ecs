...
impl<D: QueryData> Query<D> {
    ...
    pub fn iter<'a>(&self) -> impl Iterator<Item = D::ItemRef<'a>> {
        ...
        ecs
            ...
            .map(|entity| unsafe { D::fetch_ref(&ecs.component_manager, entity, &self.cached_component_indices) })
    }
    ...
    pub fn iter_mut<'a>(&mut self) -> impl Iterator<Item = D::ItemMut<'a>> {
        ...
        ecs
            ...
            .flat_map(|(_, entities)| entities.iter().copied())
            .map(|entity| unsafe { D::fetch_mut(&ecs.component_manager, entity, &self.cached_component_indices) })
    }
    ...
    pub fn get(&self, entity: Entity) -> Option<D::ItemRef<'_>> {
        ...
        Some(unsafe { D::fetch_ref(&ecs.component_manager, entity, &self.cached_component_indices) })
    }
    ...
    pub fn get_mut(&mut self, entity: Entity) -> Option<D::ItemMut<'_>> {
        ...
        Some(unsafe { D::fetch_mut(&ecs.component_manager, entity, &self.cached_component_indices) })
    }
...
pub trait QueryItem {
    type ItemRef<'a>;
    type ItemMut<'a>;
    unsafe fn fetch_ref<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemRef<'a>;
    unsafe fn fetch_mut<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemMut<'a>;
...
impl<C: Component> QueryItem for &C {
    ...
    #[inline]
    unsafe fn fetch_ref<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemRef<'a> {
        unsafe { component_manager.get_component_ptr_by_index_unchecked::<C>(entity, component_index.unwrap_unchecked()).as_ref().unwrap_unchecked() }
    }
...
impl<C: Component> QueryItem for &mut C {
    ...
    #[inline]
    unsafe fn fetch_mut<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemMut<'a> {
        unsafe { component_manager.get_mut_component_ptr_by_index_unchecked::<C>(entity, component_index.unwrap_unchecked()).as_mut().unwrap_unchecked() }
    }
...
impl QueryItem for Entity {
    ...
    #[inline]
    unsafe fn fetch_ref<'a>(_: &ComponentManager, entity: Entity, _: Option<u32>) -> Self::ItemRef<'a> {
        entity
    }

    #[inline]
    unsafe fn fetch_mut<'a>(_: &ComponentManager, entity: Entity, _: Option<u32>) -> Self::ItemMut<'a> {
        entity
    }
}
...
impl<C: Component> QueryItem for Option<&C> {
    ...
    #[inline]
    unsafe fn fetch_ref<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemRef<'a> {
        unsafe { component_manager.get_component_ptr_by_index::<C>(entity, component_index.unwrap_unchecked()).map(|f| f.as_ref().unwrap_unchecked()) }
    }
...
impl<C: Component> QueryItem for Option<&mut C> {
    ...
    #[inline]
    unsafe fn fetch_mut<'a>(component_manager: &ComponentManager, entity: Entity, component_index: Option<u32>) -> Self::ItemMut<'a> {
        unsafe { component_manager.get_mut_component_ptr_by_index::<C>(entity, component_index.unwrap_unchecked()).map(|f| f.as_mut().unwrap_unchecked()) }
    }
...
pub trait QueryData {
    type ItemRef<'a>;
    type ItemMut<'a>;
    unsafe fn fetch_ref<'a>(component_manager: &ComponentManager, entity: Entity, component_indices: &[Option<u32>; QUERY_MAX_VARIADIC_COUNT]) -> Self::ItemRef<'a>;
    unsafe fn fetch_mut<'a>(component_manager: &ComponentManager, entity: Entity, component_indices: &[Option<u32>; QUERY_MAX_VARIADIC_COUNT]) -> Self::ItemMut<'a>;
    fn join_component_signature(component_signature: &mut HashSet<TypeId>);
    fn join_required_component_signature(component_signature: &mut HashSet<TypeId>);
    fn join_component_access(component_access: &mut Access);
